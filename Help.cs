        ...
        public static void ShowHelp(string topic = "")
        {
            ...
            switch (topic)
            {
                case "":
                    ...
                    cw("1. Расчёты ведутся в double, которые не обладают бесконечной точности. " +
                       "При реализации алгоритмов не принимались меры по уменьшнию накапливающийся ошибки, " +
                       "так что могут быть связанные с этим проблемы.");
                    cw("2. Побочный эффект расчетов в double заключается в том, что, например," +
                       "разрешено деление на ноль. Я посчитал, что не имеет смысла исправлять это поведение.");
                    cw("3. Большие матрицы не выводятся полностью, только некоторая их часть. " +
